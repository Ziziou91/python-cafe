...
import pytest
from src.user_input import validate_input, find_user_input_in_valid_inputs, create_amend_item_list, handle_owner_stock_inputs, handle_owner_stock_product_inputs
...
@pytest.mark.parametrize(
    ...
    (
        ("exit", "owner_stock", "exit"),
        ("exit","owner_stock",  "exit"),
        ("help", "owner_stock", "help"),
        ("help","owner_stock",  "help"),
        ("about", "amend_menu", "about"),
        ("amend pasta", "owner_stock", ["amend", "pasta"]),
        ("remove pasta", "owner_product", ["remove", "pasta"]),
        ("remove pasta", "owner_product", ["remove", "pasta"]),
        ("add hot dog", "owner_product", ["add", "hot dog"]),
        ("add cheese toastie", "owner_product", ["add", "cheese toastie"]),
        ("  add crisps  ", "owner_product", ["add", "crisps"]),



    )
)
def test_handle_owner_stock_product_handles_inputs(input_str, called_from, expected):
    assert handle_owner_stock_product_inputs(input_str, "", called_from) == expected
...
def test_handle_owner_stock_product_handles_capitalisation(input_str, called_from,  expected):
    assert handle_owner_stock_product_inputs(input_str, "", called_from) == expected
